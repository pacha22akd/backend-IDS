...
def contar_socios(filtro_nombre=None, filtro_activo=None):
    query_str = "SELECT COUNT(*) AS total FROM socios WHERE 1=1"
    parametros = {}

    if filtro_nombre:
        query_str += " AND LOWER(nombre) LIKE LOWER(:filtro)"
        parametros["filtro"] = f"%{filtro_nombre}%"

    if filtro_activo is not None:
        query_str += " AND activo = :activo"
        parametros["activo"] = filtro_activo
...
def obtener_socios_paginados(limit, offset, filtro_nombre=None, filtro_activo=None):
